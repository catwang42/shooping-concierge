<template>
  <div ref="logRef" class="server-log-curation">
    <h4 class="text-medium-21">
      <span class="icon-shapes-container"
        ><IconShapes class="icon-shapes"
      /></span>
      Executed multimodal item curation:
    </h4>
    ...
    <p class="text-medium-18">
      Selected
      <span class="gradient found-item-count">{{ log.selectedItemCount }}</span>
      Items from
      <span class="gradient total-item-count">{{ log.totalItemCount }}</span>
      Total items in
      <span class="gradient elapsed-time">{{ log.elapsedTime }}</span> seconds
    </p>
  </div>
</template>
