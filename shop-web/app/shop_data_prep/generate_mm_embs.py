...
WHERE
  MOD(CAST(SUBSTR(id, 12) AS INT64), 5) = 3
...
def generate_mm_embeddings(items: list[dict[str, Any]]) -> list[list[float]]:
    ...
    # Download item image
    images = [Image(image_bytes=load_item_image(item["id"], 200, 200)) for item in items]
...
print(f"\nCreated mm embedding folder: gs://{GCS_BUCKET}/{GCS_MM_EMB_PATH}")
...
if __name__ == "__main__":
    ...
    print(f"\nCreated mm embeddings on folder: gs://{GCS_BUCKET}/{GCS_MM_EMB_PATH}")
